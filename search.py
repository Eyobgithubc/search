        ...
        if state not in visited:
            ...
            for successor in reversed(successors):
                if successor[0] not in visited:
                    ...
                    if problem.isWall(new_state[0], new_state[1]):
                        continue
...
def breadthFirstSearch(problem: SearchProblem):
    ...
    while queue:
        ...
        if problem.isGoalState(node):
            solution = path
            break
        for child_node, direction, cost in problem.getSuccessors(node):
            if child_node not in visited and child_node not in parents:
        ...
        for child, child_direction, child_cost in problem.getSuccessors(node):
            if child not in visited:
                ...
                if child not in costs or new_cost < costs[child]:
                    fringe.append((child, child_direction, new_cost))
                    parents[child] = node
